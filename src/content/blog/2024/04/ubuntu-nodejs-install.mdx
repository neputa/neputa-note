---
title: Ubuntu22.04にnode.jsとnpmの最新バージョンをインストールする
description: Ubuntu22.04LTS on WSL2に、Node.jsとnpmの最新バージョンをインストールする。UbuntuリポジトリのNode.jsはバージョンが古い。latestバージョンをインストールする手順をまとめる。最後に、個人的な当初のNode.jsインストールの目的、PurgeCSS導入手順を備忘録として残す。
