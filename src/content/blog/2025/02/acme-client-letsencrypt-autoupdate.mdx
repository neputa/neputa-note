...
RUN apt-get update && apt-get install -y certbot // [!code ++]

COPY ./nginx.conf /etc/nginx/nginx.conf
COPY ./site.conf.template /etc/nginx/templates/site.conf.template

RUN mkdir -p /var/www/certbot // [!code ++]

CMD [ "nginx", "-g", "daemon off;" ]
```

```apache nginx/site.conf.template
upstream app {
    server app:5000;
}
...
server {
    ...
    location /.well-known/acme-challenge/ { // [!code ++]
        root /var/www/certbot; // [!code ++]
    } // [!code ++]
}
...
services:
  ...
  nginx:
    ...
    volumes:
      ...
      - /var/www/certbot:/var/www/certbot // [!code ++]
    ports:
      - "80:80"
      - "443:443"
    networks:
  ...
  certbot: // [!code ++]
    image: certbot/certbot // [!code ++]
    volumes: // [!code ++]
      - /etc/letsencrypt:/etc/letsencrypt // [!code ++]
      - /var/www/certbot:/var/www/certbot // [!code ++]
    entrypoint: "/bin/sh -c 'trap exit TERM; while :; do certbot renew; sleep 12h; done'" // [!code ++]
